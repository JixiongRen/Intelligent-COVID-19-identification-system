package com.example.myapplication2.Wave;
...
public class WaveCanvas {
    ...
    public void Start(AudioRecord audioRecord, int recBufSize, SurfaceView sfv
        ...
        savePcmPath = audioName +"demo.pcm";//大致目录
        saveWavPath = path;//精确目录
//        saveWavPath = path + audioName +".wav";
    ...
    public  void init(){
        ...
        center.setColor(Color.rgb(199, 84, 80));// 画笔为color
...
//                Pcm2Wav p2w = new Pcm2Wav();//将pcm格式转换成wav 其实就尼玛加了一个44字节的头信息
                convertAudioFiles(savePcmPath, saveWavPath);
                if (file2wav.exists()) {
                    file2wav.delete();
                }
            } catch (Throwable t) {
