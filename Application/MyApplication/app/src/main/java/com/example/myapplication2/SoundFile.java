...
public class SoundFile {
    ...
    public static SoundFile create(String fileName,
        ...
        if (!f.exists()) {
            throw new java.io.FileNotFoundException(fileName);//这里报错没有找到目标wav文件
        }
...
//        outputStream.write(WAVHeader.getWAVHeader(mSampleRate, mChannels, numSamples));
        outputStream.write(setHeader(mSampleRate, mChannels, numSamples));
    ...
    private byte[] setHeader(int sampleRate, int numChannels, int numSamples) {
        mSampleRate = sampleRate;
        mChannels = numChannels;
        mNumSamples = numSamples;
        int mNumBytesPerSample = 2 * mChannels;  // assuming 2 bytes per sample (for 1 channel)
        byte[] mHeader;
